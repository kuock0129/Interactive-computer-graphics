    ...
    for (let t = 0; t < weathering; t += 1) {
        ...
        for (let i = 0; i < gridSize; i += 1) {
            for (let j = 0; j < gridSize; j += 1) {
                ...
                // average the neighbors from 4 directions
                if (i > 0) {
                    sumHeight += g.attributes[0][cur-gridSize][2]
                    count += 1
                }
                if (i + 1 < gridSize) {
                    sumHeight += g.attributes[0][cur+gridSize][2]
                ...
                if (j > 0) {
                    sumHeight += g.attributes[0][cur-1][2]
                    count += 1
                }
                if (j + 1 < gridSize) {
                    sumHeight += g.attributes[0][cur+1][2]
                ...
                let height = (g.attributes[0][cur][2] + (sumHeight/count)) / 2
                newHeights.push(height)
            }
        }
        let n = g.attributes[0].length
        for (let i = 0; i < n; i += 1) {
            g.attributes[0][i][2] = newHeights[i]
        }
    }
    // normalize height
...
window.addEventListener('load', async (event) => {
    ...
    const gridSize = 100, faults = 100, weathering = 10
